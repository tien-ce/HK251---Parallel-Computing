        ...
        for (int j = 0 ; j < cols; j++) {
            ...
            for (int ki = -1; ki <=1 ; ki ++){
                for (int kj = -1; kj <= 1; kj ++){
                    ...
                    float tmp = (i +ki >=0 && j + kj >=0 && i + ki < rows && j + kj < cols) ? input [i + ki][j + kj] : 30;
...
void v1_parallel_convolution(float **input, float **output, int rows, int cols) {
    // To be implemented
    #pragma omp parallel for num_threads(num_threads) default (none) shared(input, output, kernel, rows, cols)
    for (int i = 0; i < rows; i++) {
        for (int j = 0; j < cols; j++) {
            float sum = 0.0;
            // Convolution operation
